...
export class ArticleService {
	...
	async findAll(currentUserId: number, query: any): Promise<ArticlesResponseInterfce> {
		...
		if (currentUserId) {
			const currentUser = await this.userRepository
				.findOne(currentUserId, { relations: ['favorites'] });
	...
	async getFeed(currentUserId: number, query: any):
		...
		const followingIds = follows.map((el) => el.followingId);
	...
	async getArticleSlug(slug: string): Promise<ArticleEntity> {
		return await this.articleRepository.findOne({ slug });
	}

	async deleteArticle(slug: string, currentUserId: number): Promise<DeleteResult> {
		const article = await this.getArticleSlug(slug);
		if (!article) {
	...
	buildArticleResponse(article: ArticleEntity): ArticleResponseInterfce {
		return { article };
	}
